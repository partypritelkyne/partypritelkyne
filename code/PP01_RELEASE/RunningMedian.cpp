...
RunningMedian::RunningMedian(const uint8_t size)
{
  ...
  _ar = (long *) malloc(_size * sizeof(long));
...
long RunningMedian::getAverage(uint8_t nMedians)
{
...
long RunningMedian::getElement(const uint8_t n)
{
...
long RunningMedian::getSortedElement(const uint8_t n)
{
  if ((_cnt == 0) || (n >= _cnt)) return NAN;
...
long RunningMedian::predict(const uint8_t n)
{
  if ((_cnt == 0) || (n >= _cnt/2)) return NAN;

  long med = getMedian();  // takes care of sorting !
  if (_cnt & 0x01)
  {
  ...
  {
    long f1 = (_ar[_p[_cnt/2 - n]] + _ar[_p[_cnt/2 - n - 1]])/2;
    long f2 = (_ar[_p[_cnt/2 + n]] + _ar[_p[_cnt/2 + n - 1]])/2;
