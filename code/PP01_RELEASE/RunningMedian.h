...
public:
  ...
  long getAverage();                 // returns average of the values in the internal buffer
  long getAverage(uint8_t nMedian);  // returns average of the middle nMedian values, removes noise from outliers
  long getHighest() { return getSortedElement(_cnt - 1); };
  long getLowest()  { return getSortedElement(0); };

  long getElement(const uint8_t n);        // get n'th element from the values in time order
  long getSortedElement(const uint8_t n);  // get n'th element from the values in size order
  long predict(const uint8_t n);           // predict the max change of median after n additions
